...
pub async fn handle(api_url: &str, command: MetricsCommands, format: OutputFormat) -> anyhow::Result<()> {
    ...
    match command {
        ...
        MetricsCommands::Query { name, hours, aggregation, interval } => {
            let now = Utc::now();
            ...
            match format {
                ...
                OutputFormat::Compact => {
                    if let Some(data) = resp.get("data").and_then(|d| d.as_array()) {
                        for point in data {
                            let ts = point.get("timestamp").and_then(|t| t.as_str()).unwrap_or("");
